...
public class Scorer
{
	...
	public Scorer(Points input, Clusters clusters)
	{
		...
		randIndex = (float) (truePositives + trueNegatives) / allPairs;
	}
	...
	private static int getNumberOfTruePositives(Clusters clusters)
	{
		...
		for (final Points cluster : clusters)
		{
			for (int i = 0; i < cluster.size(); i++)
			{
				...
				{
					if (cluster.get(i).trueClusterId == cluster.get(j).trueClusterId)
					{
						result++;
					}
				}
		...
		for (int i = 0; i < clusters.size(); i++)
		{
			for (final Point p : clusters.get(i))
			{
				for (int j = i + 1; j < clusters.size(); j++)
				{
					...
					{
						if (p.trueClusterId != p2.trueClusterId)
						{
							result++;
						}
					}
