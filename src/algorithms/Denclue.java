...
public class Denclue extends ClusteringAlgorithm
{
	...
	public Denclue(Points input, float sigma, int minPts)
	{
		super(input);
		this.SIGMA = sigma;
		this.MIN_PTS = minPts;
		hyperSpace = new HyperSpace(SIGMA, MIN_PTS);
	}
	...
	{
		/*
		 * TODO: optimize maxIter if needed
		 */
		int maxIter = 10;
		/*
		 * TODO: optimize delta if needed
		 */
		final double delta = 0.005;
		Point currentAttractor = new Point(point.params, point.trueClusterId, -1); // serial number parameter for DBCV  
		...
		do
		{
			...
			final Point gradPoint = new Point(currentGradient, 0, -1); // -1 for DBCV
